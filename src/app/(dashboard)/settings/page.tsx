    ...
    <div className="space-y-4 animate-fade-in max-w-3xl">
      ...
      <div className="p-4 rounded-xl bg-indigo-500/10 border border-indigo-500/20 mb-6">
        <h3 className="text-sm font-semibold text-indigo-400 mb-2">
          {settings?.shopify_shop_domain ? "Reconnect Shopify Store" : "Connect Shopify Store"}
        </h3>
        <p className="text-xs text-zinc-400 mb-3">
          {settings?.shopify_shop_domain 
            ? `Currently connected to: ${settings.shopify_shop_domain}. Enter your Shopify domain to reinstall the app or reconnect your API token.`
        ...
        <form action="/api/auth/shopify" method="GET" className="flex gap-2">
          <input 
            type="text" 
            name="shop" 
            defaultValue={settings?.shopify_shop_domain || ""}
          ...
          <button 
            type="submit"
            className="bg-indigo-500 hover:bg-indigo-600 text-white px-4 py-2 rounded-md text-sm font-medium transition-colors whitespace-nowrap"
          >
            {settings?.shopify_shop_domain ? "Reinstall App" : "Install App"}
          </button>
        </form>
      </div>
