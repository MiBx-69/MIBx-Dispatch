...
interface DeliveredReportingHeaderProps {
...
export function DeliveredReportingHeader({
  stats,
  onOpenImportModal,
}: DeliveredReportingHeaderProps) {
  ...
  const handleExport = () => {
    ...
    window.open(`/api/deliveries/export?${params.toString()}`, "_blank");
  };

  const activeLabel = DATE_PRESETS.find((p) => p.id === activeDateFilter)?.label || "All Time";

  const totalCount = stats?.totalDeliveredCount ?? 0;
  const totalAmount = stats?.totalDeliveredAmount ?? 0;
  const aov = stats?.deliveredAOV ?? (totalCount > 0 ? Math.round(totalAmount / totalCount) : 0);
  const totalItems = stats?.totalDeliveredItems ?? 0;
    ...
    <div className="bg-zinc-900/60 border border-zinc-800/80 rounded-xl p-2.5 sm:p-3 space-y-2 mb-1">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5 ml-auto">
          <button
            ...
            title="Download CSV report of delivered orders"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-1.5 sm:gap-2">
        ...
        <div className="bg-zinc-950/50 border border-zinc-800/60 rounded-lg px-2.5 py-1.5 flex items-center justify-between">
          <div>
            <p className="text-[10px] text-zinc-400 font-normal">Delivered</p>
            <p className="text-xs sm:text-sm font-semibold text-zinc-100">
              {totalCount.toLocaleString()} <span className="text-[10px] text-zinc-500 font-normal">orders</span>
            </p>
          </div>
          <PackageCheck size={14} className="text-emerald-400/70" />
        </div>

        {/* Total Delivered Value / Revenue */}
        <div className="bg-zinc-950/50 border border-zinc-800/60 rounded-lg px-2.5 py-1.5 flex items-center justify-between">
          <div>
            <p className="text-[10px] text-zinc-400 font-normal">Total Revenue</p>
        ...
        <div className="bg-zinc-950/50 border border-zinc-800/60 rounded-lg px-2.5 py-1.5 flex items-center justify-between">
          <div>
            <p className="text-[10px] text-zinc-400 font-normal">Average Order (AOV)</p>
        ...
        <div className="bg-zinc-950/50 border border-zinc-800/60 rounded-lg px-2.5 py-1.5 flex items-center justify-between">
          <div>
            <p className="text-[10px] text-zinc-400 font-normal">Delivered Items</p>
            <p className="text-xs sm:text-sm font-semibold text-zinc-200">
              {totalItems.toLocaleString()}{" "}
              <span className="text-[10px] text-zinc-500 font-normal">units</span>
            </p>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2 pt-1">
          <div className="flex items-center gap-1.5 text-[11px] text-zinc-400">
            ...
            <input
              ...
              className="bg-zinc-950 border border-zinc-700 text-zinc-200 text-[11px] rounded px-2 py-0.5 outline-none focus:border-emerald-500"
            />
          </div>
          <div className="flex items-center gap-1.5 text-[11px] text-zinc-400">
            <span>To:</span>
          ...
          <button
            ...
            className="px-2.5 py-0.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded text-[11px] font-medium transition-colors disabled:opacity-50"
          >
